...
export interface Precision {
  fmt_precision(precision: number): R;
}
...
export class Formatter {
  ...
  public write_fmt(str: string, ...args: Array<_>): R {
    ...
    try {return this.write(
      str.replace(
        ...
        (
          ...
          if (alternate && $modifier === '') { throw new Error('no implementation for `Alternate & Display`'); }
          ...
          if ($modifier === '' || $modifier === undefined) {
            ...
            if (alternate) {
              assert_has<DebugAlternate>(obj, ['fmt_debug_alternate'], 'DebugAlternate');
          ...
          if ($align_count !== undefined && $align_type !== undefined) {
            ...
            if (Number.isNaN(count)) { throw new Error('invalid alignment count'); }
    ...
    ); } catch (e) { return this.result = Err(e as Error); }
  }
  ...
  public finish(): Result<string, Error> {
    if (this.result.is_err()) { return Err(this.result.unwrap_err()); }
    return Ok(this.r_buf);
  }
}

function assert_has<T>(u: unknown, keys: Array<keyof T>, name: string): asserts u is T {
...
export class DebugList {
  ...
  public finish(): R {
    return this.formatter.write(`[${this.parts.map(x=>x.fmt_debug()).join(', ')}]`);
  }
}
...
export class DebugMap {
  ...
  public key(key: Debug) {
    this.dangling = key;
    return this;
  }

  public value(value: Debug) {
  ...
  public entry(key: Debug, value: Debug) {
    if (this.dangling !== undefined) {
      panic('cannot add entry with dangling key `' + this.dangling.fmt_debug() + '`');
    }
  ...
  public entries<K extends Debug, V extends Debug>(entries: Iterable<[K, V]>): this {
  ...
  public finish(): R {
    ...
    for (const [k, v] of this.parts) {
      this.formatter.write_fmt("{} {:?}: {:?}", had_before ? ',' : '', k, v);
      had_before = true;
    }
